...
try {
  ...
  const orderItemsDb = new DatabaseSync(dbPath);
  ...
  const dbPaymentCentMismatch = new DatabaseSync(dbPath);
  dbPaymentCentMismatch.prepare("UPDATE payments SET amount=276000.01 WHERE id=?").run(retryPaymentId);
  dbPaymentCentMismatch.close();
  const paymentCentMismatch = await webhook(prodPort, "payment", { event_id: `outbox-payment-cent-mismatch-${Date.now()}`, order_id: orderId, payment_id: retryPaymentId, status: "paid", amount: 276000, provider_transaction_id: "PROVIDER-TX-CENT-MISMATCH" }, baseEnv.PAYMENT_WEBHOOK_SECRET);
  add(paymentCentMismatch.status === 409, "支付回调一分钱错配阻断", `HTTP ${paymentCentMismatch.status}`);
  const dbPaymentCentRestore = new DatabaseSync(dbPath);
  dbPaymentCentRestore.prepare("UPDATE payments SET amount=276000 WHERE id=?").run(retryPaymentId);
  dbPaymentCentRestore.close();
  ...
  const acceptanceBeforeDelivery = await request(prodPort, `/api/v1/trades/${orderId}/accept`, buyerToken, { result: "accepted", accepted_items: fullAcceptanceItems, evidence: "尚未送达的验收尝试" }, "outbox-accept-before-delivery");
  ...
  const scalarAcceptance = await request(prodPort, `/api/v1/trades/${orderId}/accept`, buyerToken, { result: "accepted", accepted_qty: 4288, evidence: "不同单位不得合并验收" }, "outbox-accept-scalar-reject");
  add(scalarAcceptance.status === 400, "生产禁止合并数量验收", `HTTP ${scalarAcceptance.status}`);
  const partialAcceptance = await request(prodPort, `/api/v1/trades/${orderId}/accept`, buyerToken, { result: "accepted", accepted_items: [{ order_item_id: Number(productionItems[0]?.id), accepted_qty: 1, evidence: "部分到货不得直接合格" }], evidence: "部分到货不得直接合格" }, "outbox-accept-partial-reject");
  ...
  const dbInvoiceCentMismatch = new DatabaseSync(dbPath);
  dbInvoiceCentMismatch.prepare("UPDATE invoices SET amount=276000.01 WHERE order_id=?").run(orderId);
  dbInvoiceCentMismatch.close();
  const invoiceCentMismatch = await request(prodPort, `/api/v1/trades/${orderId}/invoice`, supplierToken, { amount: 276000.01, seller_credit_code: "91360722MA8V85013X", buyer_credit_code: "91420100MA8V85013Y", tax_rate: 0.09, invoice_type: "增值税电子普通发票", tax_category_code: "农业产品" }, "outbox-invoice-cent-mismatch");
  add(invoiceCentMismatch.status === 409, "发票金额一分钱错配阻断", `HTTP ${invoiceCentMismatch.status}`);
  const dbInvoiceCentRestore = new DatabaseSync(dbPath);
  dbInvoiceCentRestore.prepare("UPDATE invoices SET amount=276000 WHERE order_id=?").run(orderId);
  dbInvoiceCentRestore.close();
  ...
  const releaseMissingIssuedAt = await webhook(prodPort, "payment", { event_id: `outbox-payment-release-missing-issued-at-${Date.now()}`, action: "release", order_id: orderId, payment_id: retryPaymentId, status: "paid", amount: 276000, provider_transaction_id: "PROVIDER-RELEASE-MISSING-ISSUED-AT" }, baseEnv.PAYMENT_WEBHOOK_SECRET);
  add(releaseMissingIssuedAt.status === 409, "缺少发票签发时间禁止分账", `HTTP ${releaseMissingIssuedAt.status}`);
  const dbSettlementRefConflict = new DatabaseSync(dbPath);
  dbSettlementRefConflict.prepare("UPDATE invoices SET status='已开具',invoice_no='PROD-INVOICE-REF-CONFLICT',issued_at=?,amount=276000 WHERE order_id=?").run(t, orderId);
  ...
  dbSettlementRefRestore.prepare("UPDATE invoices SET status='待开具',invoice_no=NULL,issued_at=NULL WHERE order_id=?").run(orderId);
  ...
  dbAmountMismatch.prepare("UPDATE invoices SET status='已开具',invoice_no='PROD-INVOICE-AMOUNT-MISMATCH',issued_at=?,amount=? WHERE order_id=?").run(t, 276000, orderId);
  ...
  dbRestoreAmount.prepare("UPDATE invoices SET status='待开具',invoice_no=NULL,issued_at=NULL WHERE order_id=?").run(orderId);
  ...
  const refundOutboxDb = new DatabaseSync(dbPath);
  const refundOutbox = refundOutboxDb.prepare("SELECT payload FROM institution_outbox WHERE provider='payment' AND command_type='refund' ORDER BY created_at DESC LIMIT 1").get();
  refundOutboxDb.close();
  const refundCommand = refundOutbox ? JSON.parse(refundOutbox.payload) : null;
  add(Boolean(refundCommand?.payer?.credit_code && refundCommand?.payee?.credit_code && refundCommand?.money?.amount === 100), "生产退款 Outbox 主体字段完整", "退款命令包含付款方、收款方统一社会信用代码和精确金额");
  ...
  dbAfter.prepare("UPDATE acceptances SET result='accepted',accepted_qty=4288,accepted_at=?,evidence='生产验收回执' WHERE order_id=? AND id IN (SELECT acceptance_id FROM acceptance_items WHERE acceptance_id IN (SELECT id FROM acceptances WHERE order_id=?))").run(t, orderId, orderId);
  ...
  const redLetterId = redLetter.payload?.invoice_adjustment_id;
  add(redLetter.status === 202 && redLetter.payload?.institution_outbox?.provider === "invoice" && redLetter.payload?.institution_outbox?.command_type === "red_letter", "发票红冲先入 Outbox", `HTTP ${redLetter.status}`);
  const redLetterPending = await webhook(prodPort, "invoice", { event_id: `outbox-invoice-red-letter-pending-${Date.now()}`, action: "red_letter", adjustment_id: redLetterId, order_id: orderId, original_invoice_no: "PROD-INVOICE-001", invoice_no: "PROD-RED-001", status: "pending", amount: 100 }, baseEnv.INVOICE_WEBHOOK_SECRET);
  add(redLetterPending.status === 202, "红冲待机构处理回执落账", `HTTP ${redLetterPending.status}`);
  const settlementDuringRedLetter = await request(prodPort, `/api/v1/trades/${orderId}/settle`, financeToken, { payer_credit_code: "91420100MA8V85013Y", payee_credit_code: "91360722MA8V85013X" }, "outbox-settlement-during-invoice-red-letter");
  add(settlementDuringRedLetter.status === 409, "红冲处理中禁止分账", `HTTP ${settlementDuringRedLetter.status}`);
  const redLetterVerified = await webhook(prodPort, "invoice", { event_id: `outbox-invoice-red-letter-verified-${Date.now()}`, action: "red_letter", adjustment_id: redLetterId, order_id: orderId, original_invoice_no: "PROD-INVOICE-001", invoice_no: "PROD-RED-001", status: "verified", amount: 100, verification_ref: "INVOICE-RED-VERIFY-001" }, baseEnv.INVOICE_WEBHOOK_SECRET);
  add(redLetterVerified.status === 202 && redLetterVerified.payload?.next_action?.includes("红冲"), "红冲验真回执关闭原蓝票", `HTTP ${redLetterVerified.status}`);
  const settlementAfterRedLetter = await request(prodPort, `/api/v1/trades/${orderId}/settle`, financeToken, { payer_credit_code: "91420100MA8V85013Y", payee_credit_code: "91360722MA8V85013X" }, "outbox-settlement-after-invoice-red-letter");
  add(settlementAfterRedLetter.status === 409, "红冲完成后禁止原发票分账", `HTTP ${settlementAfterRedLetter.status}`);
  const dbRedLetterRestore = new DatabaseSync(dbPath);
  dbRedLetterRestore.prepare("DELETE FROM institution_outbox WHERE aggregate_id=?").run(redLetterId);
  dbRedLetterRestore.prepare("DELETE FROM invoice_adjustments WHERE id=?").run(redLetterId);
  dbRedLetterRestore.prepare("UPDATE invoices SET status='已开具',invoice_no='PROD-INVOICE-001',issued_at=? WHERE order_id=?").run(t, orderId);
  dbRedLetterRestore.prepare("UPDATE orders SET invoice_status='已验真' WHERE id=?").run(orderId);
  dbRedLetterRestore.close();
