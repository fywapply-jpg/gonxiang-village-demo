...
const feeReceipt = await request(`/api/v1/trades/${orderId}/platform-fee/receipt`, { method: "POST", token: financeToken, role: "finance", key: "v8530-platform-fee-receipt", body: { amount: expectedPlatformFee, payer_type: "buyer", service_contract_ref: "PFS-CONTRACT-V8530-0001", invoice_ref: "PFS-INVOICE-V8530-0001", provider: "licensed_payment", provider_transaction_id: "PFS-TXN-V8530-0001", evidence_ref: "PFS-EVIDENCE-V8530-0001" } });
add(feeReceipt.status === 201 && feeReceipt.payload?.data?.settlement?.platform_fee_collection_status === "collected" && feeReceipt.payload?.data?.platform_fee_collection?.recognized_as_revenue === true, "平台费独立收款回执落账", `HTTP ${feeReceipt.status}`);
const feeReceiptReplay = await request(`/api/v1/trades/${orderId}/platform-fee/receipt`, { method: "POST", token: financeToken, role: "finance", key: "v8530-platform-fee-receipt", body: { amount: expectedPlatformFee, payer_type: "buyer", service_contract_ref: "PFS-CONTRACT-V8530-0001", invoice_ref: "PFS-INVOICE-V8530-0001", provider: "licensed_payment", provider_transaction_id: "PFS-TXN-V8530-0001", evidence_ref: "PFS-EVIDENCE-V8530-0001" } });
add(feeReceiptReplay.status === 201 && feeReceiptReplay.payload?.data?.platform_fee_collection?.status === "collected", "平台费收款回执幂等重放", `HTTP ${feeReceiptReplay.status}`);
