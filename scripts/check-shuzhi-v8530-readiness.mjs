...
add(serverSource.includes("order_delivery_constraints") && serverSource.includes("productionDeliveryConstraint") && serverSource.includes("生产订单必须提供有效的收货地经纬度") && serverSource.includes("已达到当日服务上限") && serverSource.includes("生产服务区域只能由后台商户管理岗位维护") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产订单超服务半径阻断") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产只读岗位禁止维护服务区域") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产订单超过日单量阻断") ? "pass" : "fail", "生产服务半径订单闸门", "正式订单创建必须固化收货坐标、服务距离、日单量和证据，区域维护由后台岗位负责，超半径或超量不能直接成交");
