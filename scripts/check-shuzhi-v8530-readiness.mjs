...
add(serverSource.includes("settlement_records") && serverSource.includes("SETTLE_TRADE") && serverSource.includes("验收合格前不得结算") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("订单/托管/发票金额不一致") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票与订单金额不一致禁止开票") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("机构分账流水号跨交易复用阻断") && serverSource.includes("合同、验收、发票和托管金额未全部一致") && serverSource.includes("发票金额与订单金额不一致，禁止机构回调落账") && serverSource.includes("机构分账流水号已绑定其他交易") && serverSource.includes("allowedSettlementModels") && serverSource.includes("SHUZHI_PAYMENT_READY") ? "pass" : "fail", "结算对账闸门", "合同、验收、开票、机构回调和托管资金齐备、金额逐分一致且机构分账流水号未跨交易复用后才允许最终关账");
