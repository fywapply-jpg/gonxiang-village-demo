...
add(serverSource.includes("所有运单须经物流机构确认已送达且无异常后才能验收") && serverSource.includes("shipmentSummary") && serverSource.includes("生产全量合格验收数量必须等于订单总量") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产未送达禁止提前验收") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产送达后才允许验收") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产部分数量不得直接标记合格") ? "pass" : "fail", "物流与全量验收闸门", "生产最终验收必须等待所有关联运单已送达且无异常，并覆盖订单总量；部分到货只能走争议/冻结，回归脚本覆盖送达前、部分数量拒绝与全量放行");
