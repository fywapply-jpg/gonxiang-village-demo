...
add(serverSource.includes("所有运单须经物流机构确认已送达且无异常后才能验收") && serverSource.includes("shipmentSummary") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产未送达禁止提前验收") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("生产送达后才允许验收") ? "pass" : "fail", "物流送达验收闸门", "生产最终验收必须等待所有关联运单已送达且无异常，回归脚本覆盖送达前拒绝与送达后放行");
...
add(serverSource.includes("const initialVerificationStatus = productionMode ? \"pending\" : \"verified\";") && serverSource.includes("const verificationMatch = path.match") && serverSource.includes("const merchantVerificationReady =") && serverSource.includes("COUNT(DISTINCT verification_type)") && serverSource.includes("merchantIds.some((merchantId) => !merchantVerificationReady(merchantId))") && serverSource.includes("!merchantVerificationReady(supplierId)") && serverSource.includes("!merchantVerificationReady(buyerId)") && serverSource.includes("!merchantVerificationReady(merchant.id)") && serverSource.includes("营业资质和对公账户尚未完成独立核验，不能启用商户") ? "pass" : "fail", "商户准入事实核验", "生产准入审核不伪造外部核验结论，必须由带机构和证据引用的独立 license/bank 核验记录后才能启用");
