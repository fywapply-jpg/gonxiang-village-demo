...
add(serviceTemplate.includes("ExecStartPre=/usr/bin/node /opt/shuzhi-v8530/scripts/check-shuzhi-production-bootstrap.mjs") && serviceTemplate.includes("ProtectSystem=strict") && serviceTemplate.includes("RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6") && existsSync(resolve(root, "scripts/check-shuzhi-production-bootstrap.mjs")) ? "pass" : "fail", "服务启动与隔离", "systemd 启动前执行分阶段生产核心门禁并限制文件、设备和网络权限；全量放行仍执行严格生产检查");
