...
add(serverSource.includes("发票回调金额与订单发票金额不一致") && serverSource.includes("发票金额与订单金额不一致，禁止结算") && serverSource.includes("issued_at IS NOT NULL") && serverSource.includes("TRIM(COALESCE(invoice_no,''))<>''") && serverSource.includes("禁止回调替换发票号码") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("缺少发票签发时间禁止分账") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("已开具发票号码禁止回调替换") ? "pass" : "fail", "发票金额四流核对", "开票及机构回调金额必须与订单金额一致，已开具发票号码不可被后续回调替换，并且必须有发票号码和签发时间，金额或签发事实不完整不得进入结算");
