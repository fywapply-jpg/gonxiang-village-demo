...
add(serverSource.includes("CREATE TABLE IF NOT EXISTS invoice_adjustments") && serverSource.includes("/invoice-adjustments") && serverSource.includes("红冲/作废回调缺少 adjustment_id") && serverSource.includes("发票存在红冲/作废申请或已完成调整，禁止分账") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票红冲先入 Outbox") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("红冲验真回执关闭原蓝票") ? "pass" : "fail", "发票红冲/作废出站闭环", "退款后的红冲/作废申请必须带财务复核引用、独立台账、异步机构回执和原蓝票分账闸门");
