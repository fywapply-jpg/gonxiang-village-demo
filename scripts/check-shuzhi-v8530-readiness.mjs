...
add(serverSource.includes("CREATE TABLE IF NOT EXISTS payment_refunds") && serverSource.includes("/refund") && serverSource.includes("QUEUE_PAYMENT_REFUND") && serverSource.includes("累计退款金额不得超过原支付金额") && serverSource.includes("action === \"refund\"") && serverSource.includes("退款机构已确认") ? "pass" : "fail", "生产退款出站闭环", "退款请求独立落库并进入支付 Outbox，累计退款不得超过原支付，必须由机构退款回调确认；已分账交易不允许直接退款");
