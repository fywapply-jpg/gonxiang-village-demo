...
add(serverSource.includes("settlement_records") && serverSource.includes("SETTLE_TRADE") && serverSource.includes("验收合格前不得结算") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("订单/托管/发票金额不一致") && serverSource.includes("合同、验收、发票和托管金额未全部一致") && serverSource.includes("allowedSettlementModels") && serverSource.includes("SHUZHI_PAYMENT_READY") ? "pass" : "fail", "结算对账闸门", "合同、验收、发票和托管资金齐备且订单/托管/发票金额逐分一致后才允许最终分账关账，生产必须通过支付 ready 门禁");
