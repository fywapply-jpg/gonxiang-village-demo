...
const invoicePageSource = text("work/shuzhi-v8502-source/src/pages/mine/invoice.vue");
...
add(serverProductBlock.indexOf("const media =") < serverProductBlock.indexOf('db.prepare("INSERT INTO products') && serverProductBlock.includes('db.exec("BEGIN IMMEDIATE")') && serverProductBlock.includes('db.exec("COMMIT")') ? "pass" : "fail", "商品写入原子性", "商品字段和媒体先校验，再以 IMMEDIATE 写事务写入商品、媒体及幂等记录");
...
add(serverSource.includes("CREATE TABLE IF NOT EXISTS invoice_adjustments") && serverSource.includes("/invoice-adjustments") && serverSource.includes("红冲/作废回调缺少 adjustment_id") && serverSource.includes("发票存在红冲/作废申请或已完成调整，禁止分账") && serverSource.includes('path === "/api/v1/invoices" && req.method === "GET"') && serverSource.includes("download_url") && apiSource.includes("getInvoices") && invoicePageSource.includes("loadBackendInvoices") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票红冲先入 Outbox") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票机构文件地址进入后台台账") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("红冲验真回执关闭原蓝票") ? "pass" : "fail", "发票前后台及红冲/作废闭环", "正式前台按主体读取发票台账和机构文件地址，财务调整必须带复核引用、独立台账、异步机构回执和原蓝票分账闸门");
add(serverSource.includes("amountsDiffer") && serverSource.includes("发票回调金额与订单发票金额不一致") && serverSource.includes("发票金额与订单金额不一致，禁止结算") && serverSource.includes("商品明细金额超过应开金额，禁止开票") && serverSource.includes("issued_at IS NOT NULL") && serverSource.includes("TRIM(COALESCE(invoice_no,''))<>''") && serverSource.includes("发票已有机构号码，禁止回调替换发票号码") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("支付回调一分钱错配阻断") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票金额一分钱错配阻断") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("商品明细金额超过发票金额禁止开票") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("缺少发票签发时间禁止分账") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("发票机构号码禁止回调替换") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("同号码发票验真回调落账") ? "pass" : "fail", "发票金额四流核对", "生产金额按整数分比较，开票及机构回调金额必须一致，商品明细不得超过应开发票金额，机构已返回号码后不可替换，且必须有发票号码和签发时间");
...
add(serverSource.includes("platform_fee_base") && serverSource.includes("platform_fee_collection_status") && serverSource.includes("platform_fee_collections") && serverSource.includes("platformFeeReceiptMatch") && serverSource.includes("service_contract_ref") && serverSource.includes("recognized_as_revenue") && text("scripts/check-shuzhi-v8530-settlement.mjs").includes("平台费独立收款回执落账") ? "pass" : "fail", "平台费计费与收款边界", "平台技术服务费只按商品明细净额计收；独立服务合同、发票和机构收款回执通过独立台账登记后，才允许确认平台已收收入");
add(serverSource.includes("const contractOrderMatch = order.contracts.length > 0") && serverSource.includes("const invoiceAdjustmentGate = order.invoice_adjustments.every") && serverSource.includes("const invoiceGate = invoiceAdjustmentGate && order.invoices.length > 0") && serverSource.includes("String(invoice.invoice_no || \"\").trim() !== \"\"") && serverSource.includes("const paymentReady = order.payments.some") && serverSource.includes("contract_order_match: contractOrderMatch") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("账本缺少发票号码不得显示四流通过") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("红冲处理中禁止分账") ? "pass" : "fail", "四流汇总拒绝空集合误报", "交易账本汇总必须实际存在合同、发票号码、签发时间、可用支付状态且无进行中的发票调整，不能让空数组 every() 或不完整发票误报为已通过");
add(apiSource.includes("Idempotency-Key") && apiSource.includes("newIdempotencyKey") && apiSource.includes("createTradeOrder") && apiSource.includes("createPurchaseDemand") && apiSource.includes("signTradeContract") && apiSource.includes("settleTrade") && apiSource.includes("getPurchaseDemands") && apiSource.includes("submitDemandQuote") && apiSource.includes("acceptDemandQuote") && apiSource.includes("quote_id?: string") && apiSource.includes("getInvoices") && serverSource.includes('path === "/api/v1/invoices" && req.method === "GET"') ? "pass" : "fail", "前台后端接线", "订单、发票台账、采购需求发布/报价、报价确认、合同、验收、开票、结算动作调用后台并携带幂等键及服务端金额");
...
add(["getOperationCatalog", "advanceOperation", "resetOperation", "getPlatformFeatures", "recordPlatformEvent", "getInvoices", "createPurchaseDemand", "createTradeOrder", "cancelTrade", "signTradeContract", "acceptTrade", "issueTradeInvoice", "settleTrade"].every((name) => apiSource.includes(name)) && ["/api/v1/operations/catalog", "operationAdvanceMatch", "platform/features", "platform/events", "path === \"/api/v1/purchase-demands\" && req.method === \"POST\"", "path === \"/api/v1/trades\" && req.method === \"POST\"", "path === \"/api/v1/invoices\" && req.method === \"GET\"", "cancelMatch", "contractSignMatch", "acceptMatch", "invoiceMatch", "settleMatch"].every((route) => serverSource.includes(route)) ? "pass" : "fail", "前台接口路由闭包", "采购需求发布、订单与发票台账读取、取消、业务工作流、平台事件和交易合同—验收—开票—结算调用均有对应后端路由");
...
const restoreSource = text("scripts/check-shuzhi-v8530-backup-restore.mjs");
add(existsSync(resolve(root, "scripts/check-shuzhi-v8530-backup-restore.mjs")) && restoreSource.includes("requiredTables") && restoreSource.includes("institution_outbox") && restoreSource.includes("platform_fee_collections") ? "pass" : "fail", "恢复演练工具", "可将有数据或空数据的最新备份恢复到临时 SQLite 库，并校验关键业务 schema");
add(existsSync(resolve(root, "scripts/check-shuzhi-v8530-empty-backup-restore.mjs")) && text("package.json").includes('"check:shuzhi-empty-backup-restore": "node scripts/check-shuzhi-v8530-empty-backup-restore.mjs"') && text("scripts/check-shuzhi-v8530-empty-backup-restore.mjs").includes("允许业务数据为 0") ? "pass" : "fail", "空生产库恢复回归", "首次上线的空库也能完成备份、SHA-256、integrity_check 和关键 schema 恢复演练");
const deployFiles = ["deploy/README.md", "deploy/shuzhi-v8530.env.example", "deploy/shuzhi-v8530.service", "deploy/shuzhi-v8530-institution-worker.service", "deploy/shuzhi-v8530-backup.service", "deploy/shuzhi-v8530-backup.timer", "deploy/nginx-shuzhi-v8530.conf", "deploy/nginx-shuzhi-api-v8530.conf.example", "deploy/shuzhi-v8533-capability-activation.json", "docs/数智供社-v8530-上线执行清单.md", "docs/数智供社-v8530-外部联调验收表.md", "docs/openapi/数智供社-v8533-机构适配协议-v1.openapi.json", "institution-adapters/shared/client.mjs", "institution-adapters/shared/outbox.mjs", "institution-adapters/shared/worker.mjs", "scripts/run-shuzhi-institution-worker.mjs", "scripts/create-shuzhi-production-env.mjs", "scripts/check-shuzhi-production-bootstrap.mjs", "scripts/check-shuzhi-production-activation.mjs", "scripts/check-shuzhi-production-env-staged.mjs", "scripts/check-shuzhi-v8530-empty-backup-restore.mjs", "scripts/check-shuzhi-institution-contract.mjs", "scripts/check-shuzhi-institution-client.mjs", "scripts/check-shuzhi-institution-outbox.mjs", "scripts/check-shuzhi-v8530-production-outbox.mjs", "scripts/check-shuzhi-v8530-concurrency.mjs"];
...
add(serviceTemplate.includes("ExecStartPre=/usr/bin/node /opt/shuzhi-v8530/scripts/check-shuzhi-production-bootstrap.mjs") && workerServiceTemplate.includes("ExecStartPre=/usr/bin/node /opt/shuzhi-v8530/scripts/check-shuzhi-production-bootstrap.mjs") && backupServiceTemplate.includes("ExecStartPre=/usr/bin/node /opt/shuzhi-v8530/scripts/check-shuzhi-production-bootstrap.mjs") && backupServiceTemplate.includes("After=local-fs.target shuzhi-v8530.service") && backupServiceTemplate.includes("Requires=shuzhi-v8530.service") && serviceTemplate.includes("ProtectSystem=strict") && workerServiceTemplate.includes("ProtectSystem=strict") && serviceTemplate.includes("RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6") && backupServiceTemplate.includes("backup-shuzhi-local.mjs") && backupServiceTemplate.includes("ProtectSystem=strict") && backupTimerTemplate.includes("Persistent=true") && backupTimerTemplate.includes("RandomizedDelaySec=15m") && existsSync(resolve(root, "scripts/check-shuzhi-production-bootstrap.mjs")) ? "pass" : "fail", "服务启动与隔离", "API、Outbox worker 和每日备份均使用受限 systemd 服务；备份等待 API 完成迁移后再执行，启动门禁、最小权限、备份持久化和随机错峰已纳入部署资产");
...
add(serverSource.includes("CREATE TABLE IF NOT EXISTS payment_refunds") && serverSource.includes("/refund") && serverSource.includes("QUEUE_PAYMENT_REFUND") && serverSource.includes("累计退款金额不得超过原支付金额") && serverSource.includes("部分退款") && serverSource.includes("action === \"refund\"") && serverSource.includes("action 不在允许范围") && serverSource.includes("分账机构未确认成功") && serverSource.includes("退款机构已确认") && serverSource.includes("const payer = merchantParty") && serverSource.includes("const payee = merchantParty") && text("scripts/check-shuzhi-institution-client.mjs").includes("支付退款 DTO 主体字段") ? "pass" : "fail", "生产退款出站闭环", "退款请求独立落库并进入支付 Outbox，累计退款不得超过原支付，退款命令绑定付款方/收款方主体和精确金额，并由机构回调确认；已分账交易不允许直接退款");
...
add(nginxTemplate.includes("location = /health/ready") && nginxTemplate.includes("proxy_pass http://127.0.0.1:8787/health/ready") && nginxTemplate.includes("Strict-Transport-Security") && apiNginxTemplate.includes("location = /health/live") && apiNginxTemplate.includes("location = /health/ready") && apiNginxTemplate.includes("Strict-Transport-Security") && apiNginxTemplate.includes("return 404") && text("package.json").includes('"render:shuzhi-nginx"') ? "pass" : "fail", "反向代理就绪探针", "组合部署和 EdgeOne + 独立 API 部署模板均转发健康探针，启用 HSTS，API 域名根路径不误返回前台");
...
add(existsSync(resolve(root, "scripts/build-shuzhi-mp-production.mjs")) && text("package.json").includes('"build:shuzhi-mp-production": "node scripts/build-shuzhi-mp-production.mjs"') && text("package.json").includes('"check:shuzhi-mp-production": "node scripts/check-shuzhi-mp-artifact.mjs --mode production --dir work/shuzhi-v8502-source/dist/build/mp-weixin-production"') && text("scripts/build-shuzhi-mp-production.mjs").includes('parsed.protocol === "https:"') && text("scripts/build-shuzhi-mp-production.mjs").includes("mp-weixin-production") ? "pass" : "fail", "微信生产制品隔离", "本地 HTTP 联调包与正式 HTTPS 小程序包使用不同目录；正式构建必须由真实 API 根地址触发，避免联调构建覆盖提审制品");
