...
add(serverSource.includes("const contractOrderMatch = order.contracts.length > 0") && serverSource.includes("const invoiceGate = order.invoices.length > 0") && serverSource.includes("String(invoice.invoice_no || \"\").trim() !== \"\"") && serverSource.includes("const paymentReady = order.payments.some") && serverSource.includes("contract_order_match: contractOrderMatch") && text("scripts/check-shuzhi-v8530-production-outbox.mjs").includes("账本缺少发票号码不得显示四流通过") ? "pass" : "fail", "四流汇总拒绝空集合误报", "交易账本汇总必须实际存在合同、发票号码、签发时间和可用支付状态，不能让空数组 every() 或不完整发票误报为已通过");
