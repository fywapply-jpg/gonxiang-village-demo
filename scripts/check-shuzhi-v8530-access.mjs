...
for (const path of ["/api/v1/trades", "/api/v1/trades/SZGS-2026-850901", "/api/v1/trades/SZGS-2026-850901/ledger"]) { const anon = await request(path, { token: "" }); const auth = await request(path); add(anon.status === 401, `匿名拦截 ${path}`, `HTTP ${anon.status}`); add(auth.status === 200, `授权访问 ${path}`, `HTTP ${auth.status}`); if (path.endsWith("/ledger")) add(auth.payload?.data?.reconciliation?.acceptance_invoice_gate === false && auth.payload?.data?.reconciliation?.payment_release_gate === false, "未开具发票不得显示四流放行", "演示订单发票仍为待开具，账本闸门保持 false"); }
