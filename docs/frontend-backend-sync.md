...
本地真机/开发者工具构建使用 `npm run build:shuzhi-mp`：构建脚本会优先读取 `VITE_API_BASE`，否则读取 `VITE_API_HOST`，最后自动探测 macOS 的 `en0/en1` 局域网地址并生成 `http://<局域网地址>:8787`。这样不会把历史电脑地址写入新包；手机和电脑仍必须在同一 Wi-Fi，且防火墙允许 8787。正式构建继续使用 HTTPS 地址，不得使用该本地命令发布。
